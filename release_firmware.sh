...
ASSETS=(); SUMS="$WORK/SHA256SUMS"; : > "$SUMS"

stage() {  # <src-path> <release-asset-name>
...
stage "$PLUTO" pluto.frm
stage "$LIBRE" libre.frm
[ "${#ASSETS[@]}" -gt 0 ] || { echo "error: no .frm found (looked at $PLUTO, $LIBRE)" >&2; exit 1; }
ASSETS+=("$SUMS")
...
echo "==> done: ${TAG} on ${REPO} ($(printf '%s ' "${ASSETS[@]##*/}"))"
