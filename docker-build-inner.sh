...
# Capture-control API (pluto-ctld): GPS-anchored IQ capture over ZMQ (REP :5562). The
# WRITE counterpart to pluto-zmqd -- it tunes the AD9361 and runs a PPS-gated capture,
# returning a SigMF (meta + ci16_le IQ) pair. Capture core is capture_core.c (a
# refactor of the validated DSN/sdr-stack iq_capture.c) + pps_timestamp.c; the server
# is pluto_ctld.cpp. Links libzmq + libiio (the Pluto already ships libiio 0.26). See
# docs/PLUTO_ZMQ_CTL_ICD.md.
if [ -n "$BR_CFG" ] && [ -f /build/services-src/pluto_ctld.cpp ]; then
    PKG=buildroot/package/pluto-ctld
    mkdir -p "$PKG"
    for f in pluto_ctld.cpp capture_core.c capture_core.h pps_timestamp.c pps_timestamp.h S66ctld; do
        cp "/build/services-src/$f" "$PKG/$f"
    done
    cat > "$PKG/Config.in" << 'EOF'
config BR2_PACKAGE_PLUTO_CTLD
	...
	help
	  GPS-anchored IQ capture-control daemon (ZMQ REP :5562) for the
	  Pluto+ GPS-timing firmware. Tunes the AD9361 and returns a
	  PPS-gated SigMF capture. See docs/PLUTO_ZMQ_CTL_ICD.md.
EOF
    cat > "$PKG/pluto-ctld.mk" << 'EOF'
################################################################################
# pluto-ctld -- GPS-anchored capture-control daemon (in-tree local source)
################################################################################
...
define PLUTO_CTLD_BUILD_CMDS
	...
	$(TARGET_CXX) $(TARGET_CXXFLAGS) -o $(@D)/pluto_ctld \
		$(@D)/pluto_ctld.o $(@D)/capture_core.o $(@D)/pps_timestamp.o -lzmq -liio
endef
...
$(eval $(generic-package))
EOF
    if ! grep -q 'package/pluto-ctld/Config.in' buildroot/package/Config.in; then
        echo 'source "package/pluto-ctld/Config.in"' >> buildroot/package/Config.in
    fi
    set_kcfg "$BR_CFG" BR2_PACKAGE_LIBIIO     'BR2_PACKAGE_LIBIIO=y'
    set_kcfg "$BR_CFG" BR2_PACKAGE_PLUTO_CTLD 'BR2_PACKAGE_PLUTO_CTLD=y'
    rm -rf buildroot/output/build/pluto-ctld-* 2>/dev/null || true
    info "  buildroot: pluto-ctld package created + libiio enabled (ZMQ capture-control API)"
fi
